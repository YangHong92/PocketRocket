...
def processItemSequence(spark, rawSampleDataPath):
    ...
    # filter out movies with rating below 3.5 as we expect movies with high rating are close
    ...
    # create sequences of movieId watched by a user: 11888(userId) | 296 380 344 588 593 231 ...(movieIdStr)
...
if __name__ == '__main__':
    conf = SparkConf().setAppName('ctrModel').setMaster('local').set( "spark.driver.host", "localhost" )
    spark = SparkSession.builder.config(conf=conf).getOrCreate()
    file_path = '../PocketRocket'
    rawSampleDataPath = file_path + "/webroot/server/sql/ratings.csv"
    embLength = 10
    samples = processItemSequence(spark, rawSampleDataPath)
    model = trainItem2vec(spark, samples, embLength,
                          embOutputPath=file_path + "/webroot/server/redis/item2vecEmb.csv", saveToRedis=False,
                          redisKeyPrefix="i2vEmb")
    graphEmb(samples, spark, embLength, embOutputFilename=file_path + "/webroot/server/redis/itemGraphEmb.csv",
             saveToRedis=True, redisKeyPrefix="graphEmb")
    generateUserEmb(spark, rawSampleDataPath, model, embLength,
                    embOutputPath=file_path + "/webroot/server/redis/userEmb.csv", saveToRedis=False,
                    redisKeyPrefix="uEmb")
